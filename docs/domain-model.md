...
| **Slot spec** | A slot's definition of done: the deliverable set (formats, optionally a due date). Optional — a slot without a spec behaves as today. With one, "what is missing" becomes computable and the agent can say so. Proposed from observed patterns, only ever confirmed by a human. |
| **Mark** | A derived attention signal on a card or strip ("draft ready", "1 question", "stalled 4 days"). Computed at read time from state — never stored, so never stale. Only dismissals of marks are stored (they are facts about the user). |
| **Agent draft** | A version authored by the agent: a generated fix for *mechanical* defects, entering the tree as an ordinary branch (dashed in the UI, author = agent). One drafting pass per run, after all verdicts land. Creative defects never get drafts — they get a written fix list addressed to the designer. Discarding a draft is one click and teaches the agent to propose instead of draft for that slot. |
| **Stance** | The agent's recommendation between pickable alternatives, stated with computed facts (defects resolved, measurements) and a link to verify. A recommendation is never an approval; overriding it is logged and teaches. |
| **Question thread** | A low-confidence check turned into a question asked in context on the review page ("is this teal in brand?"), with the evidence at honest size. Ignorable forever — an unanswered question never blocks. Either answer updates the relevant tolerance or rule. |
| **Placement** | Where an upload will run (TikTok, Instagram, Web…), asked once at staging. Selects which platform safe-area and format checks apply. |
...
19. Announce → claim → prove. Cards and rails may only *announce* ("draft ready", "1 question") and *claim* (the stance, in two sentences with a link); evidence at honest size and every decision live only on the review page. A measurement the owner cannot re-derive from what is on screen is indistinguishable from an opinion, so no verdict, stance or question ships without its visible receipt (version-strip flip, swatch pair in context, overlay on the frame, timeline jump).
20. Initiative is ambient, derived, and event-driven. The agent acts on events (verdicts land → one drafting pass; recheck passes → pickable mark; third override of a rule → question), and what needs attention is *computed at read time* from state — decision 14's derivation trick applied to the agenda. No stored to-do documents, no memo panel, no scheduled "review hour"; the only clock-driven work is outbound nudges (stall, deadline), done with scheduled tasks. Nothing the agent raises ever blocks the user.
21. The agent drafts only what is reversible and mechanical, and the tree is its license: a draft is a branch — nothing overwritten, discard is one click, approval never leaves the human. Creative changes are never drafted, only specified (a fix list to the designer, a cut list to the editor). This split is per-defect-category, not per-project.
...
25. Comparing is looking, not judging: the flow view offers a compare mode over any two versions of a slot — wipe (one frame, draggable divider) and side-by-side (two panes, pan/zoom locked together) — read-only, derived entirely from the tree's nodes, with each side's verdict stated and the review screen one click away. Deciding still happens where the evidence lives (decision 19); compare exists so the owner can *see* the difference before going there. Defaults to the tips of the first two variants. (Validated in the `proto/compare-variants` branch; flicker lost.)
